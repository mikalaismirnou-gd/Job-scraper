...
def client_digest_job(client_id: int, client_name: str) -> None:
    logger.info("Building digest for client_id=%d (%s)", client_id, client_name)
    with psycopg.connect(os.environ["DATABASE_URL"]) as conn:
        matches = ranking.rank_for_client(conn, client_id)
    # TODO(step 7): replace with actual Resend email send + delivery table logging.
    logger.info("client_id=%d (%s): %d matches ready to send", client_id, client_name, len(matches))


def load_active_clients() -> list[dict]:
    with psycopg.connect(os.environ["DATABASE_URL"]) as conn:
        with conn.cursor(row_factory=dict_row) as cur:
            cur.execute("SELECT id, name, schedule FROM client WHERE status = 'active';")
            return cur.fetchall()
